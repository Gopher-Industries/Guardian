            ...
            EmailPasswordAuthService(
                ...
                authService.signIn()
                    ?.addOnSuccessListener {
                        ...
                        var database= DataBase(this@LoginActivity)
                        database.open()
                      var  login = database.get_Login_Details(emailInput)

                        if(login!=null && login.loginID!=null &&!login.loginID.equals(""))
                        {

                            if(login.loginID.equals(emailInput,true))
                            {

                                if(login.password.equals(passwordInput,true))
                                {

                                    val passwordResetDialog = AlertDialog.Builder(this@LoginActivity)

                                    with(passwordResetDialog) {
                                        setTitle(getString(R.string.text_reset_password))
                                        setMessage("You Previous Visit for Reset Password \n Your Prevoius Password Are Same Please Choose Another Password.\n Do You Want To Reset Password Again!")
                                    ...
                                    database.delete_PreviousLogin()
                                    var login = Login()
                                    login.loginID = emailInput
                                    login.password = passwordInput
                                    login.isLogin = "true"
                                    login.userType =userRole.toString()
                                    database.insert_tbl_Login(login)


                                }
                            }
                            else{
                                database.UpdateIsActive()
                                var login = Login()
                                login.loginID = emailInput
                                login.password = passwordInput
                                login.isLogin = "true"
                            ...
                            val radioButton: RadioButton = findViewById(loginRoleRadioGroup.checkedRadioButtonId)
